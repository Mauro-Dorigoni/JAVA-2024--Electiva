...
public class DataReview {
	...
	public Review getOne(Review res) throws AppException{
		PreparedStatement stmt = null;
        ResultSet rs = null;
        Review r = new Review();
        try {
			...
			throw new AppException("Error: no se pudo recuperar la reseña");
		}finally {
			try {
				if(rs!=null) {rs.close();}
				if(stmt!=null) {stmt.close();}
	...
	public LinkedList<Review> getPendientes() throws AppException{
        ...
        try {
			...
			rs=stmt.executeQuery();
	            ...
	            throw new AppException("Error: no se pudieron recuperar las reseñas pendientes");
	        } finally {
	            try {
	                if (rs != null) {
	                    rs.close();
